...
export const RecentGrid = () => {
    ...
    const { data, error } = useSWR(`/api/GET/getFlows/${address?.toLowerCase()}`, fetcher);

    if (error) return <div>An error occurred.</div>;
    if (!data) return <PathFinderLoader />;
        ...
        <div>
            {data.flows && (
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mt-4 mx-auto w-full text-black">
                        {data.flows.map((flow: any, index: number) => {
                            // Get the background color based on the index (cycling through the colors)
                            const bgColor = bgColors[index % bgColors.length]; // Assuming `flow.id` is unique

                            return (
                                <Link href={`/flow/${flow.id}`} target="_blank" key={flow.id}>
                                    <WobbleCard
                                        containerClassName={`group col-span-1 lg:col-span-1 h-[250px] ${bgColor}`}
                                        className=""
                                    >
                                        <div className="max-w-xs">
                                            <h2 className="text-left tracking-tight text-pretty group-hover:text-lg text-balance text-4xl font-semibold tracking-[-0.015em] text-white">
                                                {flow.name}
                                            </h2>
                                            <p className="mt-4 text-left text-xs text-neutral-200 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                                {flow.description}
                                            </p>
                                        </div>
                                    </WobbleCard>
